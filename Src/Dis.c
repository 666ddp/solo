...
extern Uint16 Eval_State;
extern Uint16 Run_PMSM;
extern Uint16 Run_PMSM_2;
extern Uint16 HIL_FailCode;
...
{
    EALLOW;
    GpioCtrlRegs.GPBMUX1.bit.GPIO36=0;//sdl
     GpioCtrlRegs.GPBDIR.bit.GPIO36=0;
     GpioCtrlRegs.GPBMUX1.bit.GPIO35=0;//scl
...
void CH454_I2c_WrByte( Uint16 dat )	//写一个字节数据
{
    ...
    EALLOW;   
    GpioCtrlRegs.GPBDIR.bit.GPIO36=1;//设置为输出方向
    EDIS; 
	...
	for( i = 0; i != 8; i++ )  // 输出8位数据
	{
	 ...
	 EALLOW;   
    GpioCtrlRegs.GPBDIR.bit.GPIO36=0;//设置为输入方向
    EDIS; 
    SDL_1;
	iic_delay();
...
Uint16 CH454_I2c_RdByte( void )		//读一个字节数据
{
	...
	for( i = 0; i != 8; i++ )  // 输入8位数据
	{
		...
		dat <<= 1;
		if( GpioDataRegs.GPBDAT.bit.GPIO36==1) dat++;  // 输入1位
