...
// #define HIL_HIGH_REVERSE_RPM   (-100.0f)
// #define HIL_HIGH_OVERSPEED_CONFIRM_TICKS (20)
#define HIL_SAFE_RAMP_END_INDEX 1200
#define HIL_SAFE_SPEED_REF_20  (0.0066667f)
// #define HIL_SAFE_OVERSPEED_RPM (80.0f)
...
char Rx_Buffer[192];
Uint16 Rx_Index = 0;
Uint16 Rx_LineReady = 0;  // ISR置位，表示收到完整行
...
void main(void)
{
   InitSysCtrl();//禁用看门狗避免复位，设置CPU（外部晶振/2得到CPU时钟）和外设时钟频率
...
{
    if(LocationFlag!=LocationEnd)//尚未完成初始位置定位    flag初始1 END结束0
    { Get_HallAngle();
...
// 旋转方向判定
        ...
        {MechTheta=MechTheta+_IQ(360);}
        ElecTheta = _IQmpy(_IQ(4),MechTheta);   //机械角度转电角度 _IQmpy乘法的意义
...
//速度计算器每中断加一，到达速度计算预分频值才开始计算速度
                if (SpeedLoopCount >= SpeedLoopPrescaler)
                {
                    ...
                    /* HIL overspeed/reverse protection disabled for step experiments. */
...
// =========================================================================
                    ...
                    {
                        // 【核心修改：输入端抗噪滤波】对原始速度进行极其轻微的滤波，抹平离散量化阶跃
                        ...
                        // 【核心修改：彻底重构边界层，消除零点附近非线性项激发的剧烈抽动】
                        ...
                        // 3. 更新估计转速（使用干净的理想指令IQ_Given，保留纠错逻辑）
...
// =========================================================================                                        // =========================================================================                            // =========================================================================
            ...
            // 抗积分饱和限幅，建议根据实际需要的最大角加速度重新评估这个阈值
...
//======================================================================================================
     ...
     if(Sector==1)
       ...
       if((t_01+t_02)>_IQ(1))//过调制处理，按比例缩小
       {
                         ...
                         Rx_Buffer[3] == 'D' && Rx_Buffer[4] == '3' && Rx_Buffer[5] == ','))
                {
                    ...
                    else
                    {
                        ...
                        if(seg >= param_limit)
                        {
                            ...
                            if(p[param_offset + 0] > 0.001f && p[param_offset + 0] < 1000.0f) FTSMC_K1 = p[param_offset + 0];
                            if(p[param_offset + 1] > 0.001f && p[param_offset + 1] < 1000.0f) FTSMC_K2 = p[param_offset + 1];
                            if(p[param_offset + 2] > 0.0f && p[param_offset + 2] < 1000.0f)   FTSMC_MU = p[param_offset + 2];
...
float32 HIL_HighStepSpeedRef(Uint16 index)
{
    if(index < HIL_HIGH_STEP1_END_INDEX)
    {
        return 0.0f;
    }
    if(index < HIL_HIGH_STEP2_END_INDEX)
    {
        return HIL_HIGH_SPEED_REF_100;
    }
    return HIL_HIGH_SPEED_REF_200;
}
...
float32 HIL_SafeStepSpeedRef(Uint16 index)
{
    ...
    if(index < HIL_SAFE_RAMP_END_INDEX)
    {
        return HIL_SAFE_SPEED_REF_20 * ((float32)(index - STEP2_END_INDEX) / (float32)(HIL_SAFE_RAMP_END_INDEX - STEP2_END_INDEX));
    }
    return HIL_SAFE_SPEED_REF_20;
}
