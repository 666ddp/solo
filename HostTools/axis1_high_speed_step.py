...
BASE_DIR = Path(__file__).with_name("axis1_highspeed_result")
...
def save_result(trace):
    ...
    csv_path = BASE_DIR / f"axis1_highstep_0_100_200_{timestamp}.csv"
    png_path = BASE_DIR / f"axis1_highstep_0_100_200_{timestamp}.png"
    ...
    print(f"   min={np.min(speed):.2f}, max={np.max(speed):.2f}, tail_mean={np.mean(speed[-200:]):.2f}")


def main():
    cmd = "HSTEP1," + ",".join(f"{value:.8f}" for value in BEST_PTSMC_AXIS1) + "\n"

    print("=====================================================")
    print("Starting axis-1 fixed-parameter high-speed multi-step experiment")
    print("=====================================================")

    print("\n[Axis 1 High-speed Run: 0 rpm 1.0s, 100 rpm 1.5s, 200 rpm 1.5s]")
    ...
    print("\nAxis-1 high-speed experiment complete")
