...
BASE_DIR = Path(__file__).with_name("axis2_single_result")
...
def remove_spikes(speed, max_abs=300.0, max_step=150.0):
    clean = speed.copy()
    last = clean[0]
    changed = 0
...
def save_result(trace):
    ...
    csv_path = BASE_DIR / f"axis2_multistep_0_40_80_{timestamp}.csv"
    png_path = BASE_DIR / f"axis2_multistep_0_40_80_{timestamp}.png"
    ...
    print(f"   filtered min/max={np.min(clean):.2f}/{np.max(clean):.2f}, tail_mean={np.mean(clean[len(clean)//2:]):.2f}")
    print(f"   filtered spike points: {spike_count}")


def main():
    cmd = "STEP2," + ",".join(f"{value:.8f}" for value in BEST_PI_AXIS2) + "\n"

    print("=====================================================")
    print("Starting axis-2 fixed-parameter multi-step experiment")
    print(f"Axis 2 PI Kp={BEST_PI_AXIS2[0]:.8f}, Ki={BEST_PI_AXIS2[1]:.8f}")
    print("=====================================================")

    print("\n[Axis 2 Multi-step Run: 0 rpm 1.0s, 40 rpm 1.5s, 80 rpm 1.5s]")
    ...
    print("\nAxis-2 multi-step experiment complete")
